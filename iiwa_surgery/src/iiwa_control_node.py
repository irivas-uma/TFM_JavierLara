...
class IiwaSurgeryNode:
    """
    @brief Clase que define el nodo ROS para el control del robot quirúrgico iiwa.
    """
    def __init__(self):
        """
        ...
        self.tool_length = rospy.get_param("tool_length", 0.0) # @note Longitud de la herramienta en metros.
        self.tool_orientation = rospy.get_param("tool_orientation", [0.0, 0.0, 0.0]) # @note Orientación de la herramienta en radianes.
        self.fulcrum_fi = rospy.get_param("fulcrum_fi", 0.0) # @note Ángulo de fulcro en grados.
        self.robot_ip = rospy.get_param("robot_ip", "") # @note Dirección IP del robot.
    ...
    def joint_command_callback(self, msg):
        """
        @brief Callback para recibir comandos de posición articular y posteriormente enviarlos al método move_joint de la clase iiwa_surgery.
    ...
    def pose_command_callback(self, msg):
        """
        @brief Callback para recibir comandos de posición cartesiana y posteriormente enviarlos a los métodos move_cartesian o move_cartesian_fulcrum de la clase iiwa_surgey dependiendo si el movimiento es libre o de pivoteo.
    ...
    def joint_state_callback(self, msg):
        """
        @brief Callback para recibir estados de posición articular y posteriormente publicarlos en el topic iiwa_surgery/output/joints.
    ...
    def cartesian_pose_callback(self, msg):
        """
        @brief Callback para recibir estados de posición cartesianay posteriormente publicarlos en el topic iiwa_surgery/output/ef_pose y tras trasnformación en iiwa_surgery/output/tcp_pose.
